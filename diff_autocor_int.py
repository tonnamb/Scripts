...
for c in range(len(cutoff_array)):
	...
	if not os.path.exists('diff_cutoff_{0}'.format(cutoff)):
		os.makedirs('diff_cutoff_{0}'.format(cutoff))
...
for i in range(spec_nwin):
	...
	y = estimated_autocorrelation(data.RC[i])
	for c in range(len(cutoff_array)):
		cutoff = cutoff_array[c]
		...
		plt.savefig('diff_cutoff_{0}/auto_{1}.png'.format(cutoff, i+1), bbox_inches='tight')
		plt.close()
...
for c in range(len(cutoff_array)):
	cutoff = cutoff_array[c]
	np.savetxt('diff_cutoff_{0}/diff_cutoff_{0}.txt'.format(cutoff), calc_diff_array[c])

# 0.0015 ps * 100 colvarsTrajFrequency
plt.plot(data.MeanRC, calc_diff_array[0], label='{0} ({1} ps)'.format(cutoff_array[0], cutoff_array[0]*0.0015*100))
plt.plot(data.MeanRC, calc_diff_array[1], label='{0} ({1} ps)'.format(cutoff_array[1], cutoff_array[1]*0.0015*100))
plt.plot(data.MeanRC, calc_diff_array[2], label='{0} ({1} ps)'.format(cutoff_array[2], cutoff_array[2]*0.0015*100))
plt.plot(data.MeanRC, calc_diff_array[3], label='{0} ({1} ps)'.format(cutoff_array[3], cutoff_array[3]*0.0015*100))
plt.plot(data.MeanRC, calc_diff_array[4], label='{0} ({1} ps)'.format(cutoff_array[4], cutoff_array[4]*0.0015*100))
plt.plot(data.MeanRC, calc_diff_array[5], label='{0} ({1} ps)'.format(cutoff_array[5], cutoff_array[5]*0.0015*100))
plt.ylabel(r'Diffusion coefficient ($cm^2/s$)')
plt.xlabel(r'z ($\AA$)')
plt.title(system_name)
plt.legend(frameon=False, loc=0)
plt.savefig('plot_diff_cutoff.png', bbox_inches='tight')
plt.show()
plt.close()