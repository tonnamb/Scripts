...
if not os.path.exists("ui_nostat/data"):
...
for i in runrange:
    ...
    shutil.copy("../../1.5ns/k_1.7/ui_nostat/data_unslice/{0}".format(i),"ui_nostat/data_unslice/{0}".format(i))
...
for i in runrange:
    with open(str(i) + "_window/out.colvars.traj") as file:
        ...
        print i
        line = file.readline()
        with open("ui_nostat/data_unslice/" + str(i),"a") as out:
            while line:
                rows = line.split()
                if rows[0] != '#':
...
for i in runrange:
	with open('data_unslice/' + str(i)) as fin, open('data/' + str(i), 'a') as fout:
		fout.writelines(islice(fin, None, None, 10))


os.system("ui.out -ui -T 433 -min 0 -max 50 -n 2000 -u kcal -ss 0 -r -1 -v 2 > log.txt")
...
os.system("wham.out -wham -T 433 -min 0 -max 50 -n 400 -u kcal -ss 0 -r -1 -v 2 > wham.txt")