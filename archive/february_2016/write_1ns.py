...
def writesystem(window,start,step,windowstep,kforce):
    
	os.makedirs("1ns/k_" + str(kforce) + "/" + window + "_window")
    
	shutil.copy("Ag_O1X5.5_O2X0.55.eam.fs","1ns/k_" + str(kforce) + "/" + window + "_window" + "/Ag_O1X5.5_O2X0.55.eam.fs")
	
	f = open("1ns/k_" + str(kforce) + "/" + window + "_window/system.in","w")
	...
	f.write("read_restart US.window" + window + ".1ns_nc\n\n")
	...
	f.write("restart         2000 US.window" + window + ".restart.1ns_dup US.window" + window + ".restart.1ns\n\n")
	...
	f.write("fix 			thermo_output all print 100 \"${STEP} ${PE} ${KE} ${ETOTAL} ${TEMP} ${VOL} ${PRESS} ${LX} ${LZ} ${PXX} ${PYY} ${PZZ}\" file thermo.lammps title \"# Step PotEng KinEng TotEng Temp Volume Press Lx Lz Pxx Pyy Pzz\"\n\n")
	...
	f = open("1ns/k_" + str(kforce) + "/" + window + "_window/pull_unix.in","w")
	...
	f.write("	centers " + str((float(window) - 1.0)*windowstep+2.6) + "\n")
	f.write("	targetCenters " + str((float(window) - 1.0)*windowstep+2.6) + "\n")
	...
	f = open("1ns/k_" + str(kforce) + "/" + window + "_window/run_npt.py","w")
	...
	os.makedirs("1ns/k_" + str(kforce) + "/" + window + "_window" + "/no_colvars") 
	
	shutil.copy("Ag_O1X5.5_O2X0.55.eam.fs","1ns/k_" + str(kforce) + "/" + window + "_window" + "/no_colvars/Ag_O1X5.5_O2X0.55.eam.fs")
	
	f = open("1ns/k_" + str(kforce) + "/" + window + "_window" + "/no_colvars/system.in","w")
	...
	f.write("read_restart pull.restart." + str(start+(int(window)-1)*step) + "\n\n")
	...
	f.write("restart         1 US.window" + window + ".1ns_nc US.window" + window + ".1ns_nc\n\n")
	...
	f.write("fix            1 all nvt temp 300.0 300.0 0.1 fixedpoint 0.0 0.0 0.0\n")
	f.write("fix 			thermo_output all print 1 \"${STEP} ${PE} ${KE} ${ETOTAL} ${TEMP} ${VOL} ${PRESS} ${LX} ${LZ} ${PXX} ${PYY} ${PZZ}\" file thermo.lammps title \"# Step PotEng KinEng TotEng Temp Volume Press Lx Lz Pxx Pyy Pzz\"\n\n")
	
	f.write("run             1\n")
	f.close()
	
	f = open("1ns/k_" + str(kforce) + "/" + window + "_window" + "/no_colvars/run_npt.py","w")
	...
	shutil.copy(os.pardir + "/pull_200_windows/1_IC/pull.restart." + str(start+(int(window)-1)*step),"1ns/k_" + str(kforce) + "/" + window + "_window" + "/no_colvars/pull.restart." + str(start+(int(window)-1)*step))
...
def writerun(run,start,stop,kforce):
	...
	f.write("#PBS -m bae\n")
	f.write("#PBS -N " + str(run) + "_k" + str(kforce) + "_200win_2P100_1ns_bs_US\n")
	...
	f = open("1ns/k_" + str(kforce) + "/run_window" + str(run) + ".py","w")
...
for i in range(1,202):
	writesystem(str(i),22765000,1000,0.087,0.7)

for i in range(1,51):
	writerun(i,i*4-3,i*4,0.7)

writerun(51,201,201,0.7)