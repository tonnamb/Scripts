...
EndTime = range(30000,60001,2000)
...
VaryTime = range(10000,10001,1000)
...
for i in range(Nwin):
	data[i] = np.genfromtxt("data/" + str(i+1), usecols=(0))
...
mkmean_t = np.empty([len(EndTime),len(VaryTime)])
mkstd_t = np.empty([len(EndTime),len(VaryTime)])
swtest_t = np.empty([len(EndTime),len(VaryTime)])
vntest_t = np.empty([len(EndTime),len(VaryTime)])
MinVal_t = np.empty([len(EndTime),len(VaryTime)])
MinNumSeg_t = np.empty([len(EndTime),len(VaryTime)])
MinWidth_t = np.empty([len(EndTime),len(VaryTime)])
val_t = np.empty(len(EndTime))
idx_t = np.empty(len(EndTime))
seg_t = np.empty(len(EndTime))


for EndTimeIdx in range(len(EndTime)):

	print EndTime[EndTimeIdx]
	...
	for StartTimeIdx in range(len(VaryTime)):
		...
		DataStart = data[:,(VaryTime[StartTimeIdx]):EndTime[EndTimeIdx]]
		...
		mkmean_t[EndTimeIdx,StartTimeIdx] = mkmean_s[idx]
		mkstd_t[EndTimeIdx,StartTimeIdx]  = mkstd_s[idx]
		swtest_t[EndTimeIdx,StartTimeIdx] = swtest_s[idx]
		vntest_t[EndTimeIdx,StartTimeIdx] = vntest_s[idx]
		MinVal_t[EndTimeIdx,StartTimeIdx] = val
		MinNumSeg_t[EndTimeIdx,StartTimeIdx] = NumSeg[idx]
		MinWidth_t[EndTimeIdx,StartTimeIdx]  = len(DataStart[0])/NumSeg[idx]

	val, idx = min((val, idx) for (idx, val) in enumerate(MinVal_t[EndTimeIdx,:]))
	
	val_t[EndTimeIdx] = val
	idx_t[EndTimeIdx] = idx
	seg_t[EndTimeIdx] = int(MinWidth_t[EndTimeIdx,idx])
	
	with open("stats_" + str(EndTime[EndTimeIdx]) + ".txt","w") as out:
		out.write("Use SegWidth -seg:\n")
		out.write(str(int(MinWidth_t[EndTimeIdx,idx])) + "\n\n")
		...
		out.write(str(int(mkmean_t[EndTimeIdx,idx])) + "\n\n")
		out.write("Best MK Std Fail Count:\n")
		out.write(str(int(mkstd_t[EndTimeIdx,idx])) + "\n\n")
		out.write("Best SW Fail Count:\n")
		out.write(str(int(swtest_t[EndTimeIdx,idx])) + "\n\n")
		out.write("Best VN Fail Count:\n")
		out.write(str(int(vntest_t[EndTimeIdx,idx])) + "\n\n")
		
		out.write("Start Time Vary:\n")
		out.write(str(VaryTime) + "\n\n")
		
		out.write("Trend All Fail %:\n")
		out.write(str(np.round((100*MinVal_t[EndTimeIdx,:]/(4*Nwin)),1)) + "\n\n")	
		
		
	print str(int(seg_t[EndTimeIdx]))	
	
	os.system("ui.out -ui -d data_" + str(int(EndTime[EndTimeIdx])) + " -T 300 -min -5 -max 100 -n 400 -u kcal -seg " + str(int(seg_t[EndTimeIdx])) + " -ss 10000 -r -1 -v 3 > log_" + str(EndTime[EndTimeIdx]) + ".txt")
...
for i in EndTime:
	...
	with open("plotStep.txt",'a') as out:
		out.write(str(i) + "\n")